            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
                ...
                <div className='flex flex-col'>
                    ...
                    <input onChange={(e) => setCheckInDate(e.target.value)}
                        min={new Date().toISOString().split('T')[0]} type="date" id='checkInDate' className='w-full rounded border border-gray-300 px-3 py-2 mt-1.5 outline-none' required />
                </div>
            ...
            <div className='mt-25 space-y-4'>
                {roomCommonData.map((spac, index) => (
                    <div key={index} className='flex items-start gap-2'>
                        <img src={spac.icon} alt={`${spac.title}-icon`} className='w-6.5' />
                        <div>
                            <p className='text-base'>{spac.title}</p>
                            <p className='text-gray-500'>{spac.description}</p>
                        </div>
                    </div>
                ))}
            </div>
